...
async def _project_headers(client, auth_headers) -> dict:
    ...
    resp = await client.post(
        "/api/v1/projects", json={"name": "Txn Test"},
...
async def test_assign_nonexistent_txn(client, auth_headers):
    ...
    headers = await _project_headers(client, auth_headers)
    resp = await client.post("/api/v1/transactions/assign_category", json={
...
async def test_assign_bulk_validation(client, auth_headers):
    ...
    headers = await _project_headers(client, auth_headers)
    resp = await client.post("/api/v1/transactions/assign_category_bulk", json={
        "cp_key": "",
        "cat_lvl1": "",
    }, headers=headers)
...
async def test_assign_by_ids_validation(client, auth_headers):
    ...
    headers = await _project_headers(client, auth_headers)
    resp = await client.post("/api/v1/transactions/assign_category_by_ids", json={
        "txn_ids": [],
        "cat_lvl1": "",
    }, headers=headers)
    assert resp.status_code == 400
