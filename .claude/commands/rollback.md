...
Откат кода НЕ откатывает схему БД. Миграция обратимая → `ssh dds-app "cd /opt/dds && docker compose exec backend alembic downgrade -1"`. Необратимая (`DROP COLUMN` с данными) — схему откатывать нельзя, нужен forward fix.

## 5. Верификация и postmortem
`curl -sf https://app.vyatkin-wb.ru/api/health`; запись в `memory/project_known_bugs.md`.
