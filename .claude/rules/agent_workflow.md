...
> Грузится в каждый сеанс (нет `paths` frontmatter) — потому что hook может инжектить параллелизм для любого промпта.
...
**Подробно:** `.claude/rules/_archive/lead_agent_full.md` (полная версия канона).
