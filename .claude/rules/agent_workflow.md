...
Для Фазы 2 (несколько агентов работают одновременно) используй параметр Agent tool `isolation: "worktree"`:
- Создаёт временный git worktree автоматически (вместо `scripts/worktree-start.sh`)
- Убирается сам если агент не сделал изменений
- Если изменения есть — путь и ветка возвращаются в результате для merge
- 0 конфликтов файлов между параллельными агентами

Когда использовать:
- 2+ агента редактируют код одновременно → ВСЕГДА `isolation: worktree`
- Агент только читает / запускает тесты → не нужно (работает в основной директории)
