...
description: Переезд на статусной шкале заявки (PENDING…DELIVERED + RETURNED) — где «принято по журналу» врёт после возврата и почему авто-отгрузка «строго из VEHICLE_ASSIGNED» держится только на моменте сбора кандидатов
metadata:
  type: project
---
