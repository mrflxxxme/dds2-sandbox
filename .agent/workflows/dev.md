...
## ⚠️ ОБЯЗАТЕЛЬНО после КАЖДОГО изменения

// turbo
10. **Документация** — ВСЕГДА обновляй `AGENTS.md` после любого изменения кода. Даже мелкие фиксы должны быть отражены. Это критично для понимания проекта другими ИИ-агентами.

// turbo
11. **Git push** — ВСЕГДА коммить и пушь изменения на GitHub после завершения работы:
    ```bash
    git add -A
    git commit -m "feat/fix/docs: краткое описание изменений"
    git push origin main
    ```
    Формат коммитов: `feat:` (новая функция), `fix:` (исправление), `docs:` (документация), `refactor:` (рефакторинг), `chore:` (инфра/конфиг).
