...
**Исключение — расплывчатый промпт (hook `[ТЗ-ГЕЙТ]` подсветит):** баг без указателя (URL/файл/ID), «ничего не изменилось» без данных об окружении, «переделай дизайн» без референса → задай 1–3 конкретных вопроса через AskUserQuestion ДО начала анализа. Если место однозначно из контекста сессии — работай сразу.
**Формула хорошего промпта** (напоминай юзеру при расплывчатых ТЗ): ЧТО · ГДЕ (URL/файл/ID) · КАК ПОЙМЁМ, ЧТО ГОТОВО — [docs/PROMPT_GUIDE.md](docs/PROMPT_GUIDE.md).
