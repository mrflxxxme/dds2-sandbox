...
## Перед началом задачи — задай уточняющие вопросы
Перед реализацией ЛЮБОЙ нетривиальной задачи ОБЯЗАТЕЛЬНО уточни:
1. **Какие модели/таблицы затронуты?** → прочитай DOMAIN_*.md
2. **Есть ли фронтенд-часть?** → если да, уточни UX (таблица/форма/модалка)
3. **Кэш** → нужен ли @cached? Какой TTL? Когда инвалидировать?
4. **Мультитенантность** → как фильтруется project_id? Нет ли утечки?
5. **Крайние случаи** → что если пустые данные? Дубликаты? Параллельный запуск?
6. **Миграция** → нужна ли Alembic миграция? Совместима ли с текущими данными?
7. **Тесты** → какие сценарии покрыть? Нужны ли API-тесты или unit достаточно?

НЕ начинай писать код, пока не понял задачу полностью. Лучше задать 3 вопроса, чем переделывать.
