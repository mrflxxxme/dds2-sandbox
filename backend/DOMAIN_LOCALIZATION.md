...
- **Cold-start новинок:** `pick_*` функции сортируют по `(traffic DESC, priority_score DESC, name ASC)` — traffic из истории проекта остаётся primary, priority-rank speed-карты — tiebreak. Вес сплита внутри ФО — реальный трафик заказов склада (cities_count speed-якорей только как fallback без трафика: вес 3:1 давал Калининграду 25% доли СЗФО при ~0.3% трафика). **Гарантия СЗФО** (аудит 2026-07-09): при партии ≥ `NW_GUARANTEE_MIN_PACKS`(4)×min_pack СЗФО получает минимум min_pack — floor и концентрация до target% больше не оставляют его без новинок; собирается по исходной доле ДО `concentrate_share_to_target`. Зеркало в коробах на фронте — `coldStartSeed.seedNewcomerWholeBoxes` (`NW_GUARANTEE_MIN_BOXES=4`, ≥4 коробов → 1 короб СЗФО от донора-максимума). **Мета якорей несёт гарантию (2026-07-15):** в `main_warehouses` гарантированный ФО, отрезанный концентрацией, сохраняет ИСХОДНУЮ долю (до `concentrate_share_to_target`), а не 0 — фронтовый засев (экран «Распределение машины» + коробочные новинки черновика) сеет по `share_pct` якорей, и при 0 его гарантия (фильтр `sf>0`) физически не срабатывала → чисто-новиночная машина не имела ни одной СЗФО-колонки (кейс V-0031). Прочий хвост без гарантии остаётся с 0. Восстановленная доля видна ВСЕМ потребителям `main_warehouses` (в т.ч. `filteredMainWarehouses`/`recomputeColdStartAlloc` в «Потребности по складам» — Шушары получает разбавленный слайс и у обычных SKU) — намеренно: СЗФО получает свою реальную долю везде, где раздача идёт по якорным долям; over-allocation нет (все потребители само-нормируются Σshare).
