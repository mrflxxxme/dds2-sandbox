...
- `services/cold_start_distribution_service.py` — distribute_multi + priority-rank tiebreak. Гвард пересорта новинок (`OVERSORT_SELL_THROUGH_PCT=5`): посев лежит на WB, а продажи за окно < max(1, ceil(5% от лежащего)) → авто-досев SKU остановлен (`oversort_guard`/`guard_reason` в `ColdStartTableRow`); ручной override на UI гвард перекрывает. Канал новинок ручной раскладки черновика (`DraftMatrixView` → `newcomerAlloc` в `buildDraftSkus`) питается теми же `allocations`.
- `services/localization_tariff.py` — KTR_TABLE, KRP-боксы.
