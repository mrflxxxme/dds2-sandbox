...
- `services/assembly_draft_service.py` — CRUD черновика + `commit_draft` (партиальный по package_type/newcomer) + жизненный цикл юнита (`hand_off_unit` / `revert_unit` / `commit_unit` / `set_unit_items` / `delete_unit`, helpers `_carve_unit_from_rows` / `_create_one_request`).
- `services/assembly/crud.py` — `_validate_available_for_assembly`.
- `routers/assembly.py` — CRUD + workflow + analytics.
- `routers/assembly_drafts.py` — `/api/v1/assembly/drafts` (list/get/create/update/delete; `POST /{id}/commit?package_type=&newcomer_filter=`; `POST /{id}/units/{hand-off,revert,commit,items,delete}`).
