...
| Гейты доступа на ручках | Три фабрики в `rbac.py`, все проверяют членство + роль + ключ раздела ОДНИМ запросом за участником. `require_role(min_role, page)` — на отдельную ручку (своя роль: `payment-requests/approve` — admin). `require_page(page, read_paths=…)` — на `APIRouter` целиком, роль по методу (GET — viewer, мутация — editor), ключ один на домен; `read_paths` оставляет чтением POST-ради-тела (`/card-exchange/showcase`, `/fbs/supplies/plan`). `require_page_map(pages_by_route, default)` — на `APIRouter`, у которого ручки сидят на РАЗНЫХ ключах: единственный пользователь — `routers/funnel.py` (91 ручка на семь разделов, раскладка `ROUTES_BY_PAGES` там же). Ключей у ручки может быть несколько — «достаточно любого» (`GET /funnel/data` открывают воронка, реклама и локализация). Гейт на роутере, а не на ручке, — это fail-closed: ручка, дописанная завтра, закрыта по умолчанию. Гард — `tests/test_rbac_page_gates.py`: домены разложены по `GATED_DOMAINS` / `DEFERRED_DOMAINS` / `OUT_OF_SCOPE`, новый роутер без классификации роняет тест |
