...
- **`period_mode`** (`sale` | `report`): `sale` (дефолт) атрибутирует строки по дате продажи/начисления (`COALESCE(sale_dt, rr_dt)`, accrual); `report` — по отчётному периоду недельного `realizationreport` (`date_from`/`date_to`), сходится с кабинетным «Итого к оплате» ВБ. Расхождение sale-vs-report = строки на стыке недель (продажа в одной неделе, отчёт в другой). Свитч меняет только SQL-предикат даты (`period_filter` в `wb_bdr_helpers.py`); обогащение (реклама/себестоимость/отмены) со своим окном.
- **`net_payout` (Чистая выплата)** = выплата ВБ, разнесённая по категориям. Пер-строка стартует как `to_pay − adv_sum` (отзывы уже в `to_pay`; реклама режется per-nm из трекинга). Затем `finalize_net_payout` (`bdr_enrichment.py`) **разносит НЕкатегорийный остаток** (кредиты/займы + зазор «трекинг-реклама vs финотчёт-удержание») по категориям пропорц. выручке (fallback — по модулю `to_pay`), строки-котёл без `sa_name` обнуляет. Итог: **`Σ net_payout == summary.to_pay`** (фактическая выплата ВБ) на любом проекте. **КРИТИЧНО:** без разноса проекты с крупными кредитами (напр. «Вяткин»: loan_deduction ~11%+) завышали «Чистую выплату» на сумму кредитов. Остаётся зазор ~0.1% детализация-vs-кабинет (см. `period_mode`), но кредиты больше НЕ раздувают.
...
- **Операционные расходы = опекс по типам контрагентов (из банковских платежей) + «Проценты по займам»**
  (`loan_analytics.cost_by_month_keys`, строка `opex_loan_interest`; при наличии комиссий раскрывается
  на «Проценты» / «Комиссии банков»). Стоимость денег берётся по НАЧИСЛЕНИЮ по дням с бакетом в
  календарный месяц, а не по кассе: проценты платят рвано или не платят вовсе, и по кассе месяц с
  живым долгом показал бы нулевой расход. База та же, что на дашборде займов, — цифры сходятся.
  При фильтре по бренду/артикулу строка не показывается (проценты к бренду не относятся), как и
  остальной опекс. На боевых данных за 7 мес. 2026 это 12,76 млн ₽, которых в прибыли раньше не было.
