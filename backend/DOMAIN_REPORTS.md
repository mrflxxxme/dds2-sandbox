...
- **`net_payout` (Чистая выплата)** = `to_pay − adv_sum` на строку (отзывы уже в `to_pay`, т.к. ВБ привязывает их к предмету; реклама/кредиты в финотчёте идут котлом БЕЗ предмета → режем per-nm рекламу из трекинга). **Сводка:** `summary.net_payout` переопределяется в сервисе суммой по строкам с непустой категорией (`sa_name`), т.к. `summary.to_pay` УЖЕ содержит вычет рекламного котла — иначе двойной счёт. Не сходится с кабинетом на ~0.1% (трекинг-реклама vs финотчёт-удержание + кредиты; для точного якоря нужна кабинетная цифра вручную).
