...
`loan_analytics._load(..., direction="OUTGOING")` + `income_by_month_keys`. Учредитель, занявший
под 26 % и передавший деньги своему ООО под 27 %, платит не 26 %, а разницу. В ОПиУ доход идёт
строкой «Проценты полученные» со знаком минус внутри «Процентов по займам», поэтому родитель —
НЕТТО-стоимость денег. На боевых данных проекта Вяткина за 7 мес. 2026: уплачено 10,83 млн,
получено 7,33 млн, реальная цена денег 3,50 млн.

### Начисление по дням (`services/loan_daily.py`)
`GET /loans/{id}/accrual-days?date_from=&date_to=` — расшифровка свода: тело на дату, ставка
этого дня, проценты за сутки и нарастающий итог. Считает та же `accrued_in_window` окном в одни
сутки, поэтому сумма дней сходится со сводом бит-в-бит. Тело показывается ДО движения дня —
деньги начинают работать со следующего (на этом ловились расхождения с банком: день смены
ставки и день выборки транша). Максимум 400 дней за запрос.
