...
| Зарплата | команды владеют брендами/категориями; база недели (Пн–Вс, месяц по четвергу) = net_payout БДР `period_mode='report'` × глобальная тарифная лестница; фикс-оклады периодами (`PayrollSalaryPeriod`, история изменений оклада), план выплат 10/25 M+1, официалка из выписки по `counterparty_id`, строка «ФОТ (начислено)» с подстроками в ОПиУ (исключение выплат из opex скоупится датами периодов). `models/payroll.py`, `schemas/payroll.py`, `services/payroll_service.py`, `routers/payroll.py` (page-гейт `salary`) | — |
