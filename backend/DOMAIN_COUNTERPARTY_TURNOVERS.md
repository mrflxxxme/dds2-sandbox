# Domain: COUNTERPARTY_TURNOVERS

> Auto-generated skeleton by post_stop_check hook. Заполни секции и удали этот блок.

## Назначение
TODO: краткое описание домена и ответственности

## Архитектура
```
Вопрос/действие пользователя
    ↓
routers/counterparty_turnovers.py — HTTP endpoints
    ↓
services/counterparty_turnovers_service.py — бизнес-логика
    ↓
models/ — ORM
```

## Ownership
Файлы этого домена:
- `services/counterparty_turnovers_service.py` — TODO: описание
- TODO: остальные файлы

## Ключевые модели
- TODO: основные сущности

## Бизнес-правила
- TODO: ключевые инварианты

## Кэш
- TODO: prefix кэша если используется + invalidate_project_reports() если нужно

## Известные грабли
- TODO

## Тесты
- `tests/test_counterparty_turnovers.py`
