...
async def get_fbo_supply_items(
    ...
    """Get items (orders) for a specific FBO supply."""
    try:
        items = await fbo_supply_service.get_fbo_supply_items(db, project.id, supply_id)
...
async def sync_fbo_supplies(
    ...
    """Manual full sync — fetch all supplies from WB API (last 60 days)."""
    try:
