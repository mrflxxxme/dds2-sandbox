...
async def get_day_analysis(
    ...
    """
    Day analysis report: summary, comparison with previous day,
    top products, trend, anomalies.
    """
    pid = project.id
    tax_rate = float(project.tax_rate or 6)
    ...
    # ── Helper: aggregate one day ───
    async def _day_agg(d: date):
        q = select(
        ...
        revenue = os_  # simplified (buyout mix not applicable to aggregate)
        tax = revenue * tax_rate / 100
        profit = revenue - adv - tax
    ...
    # ── Top products for the target day ───
    q_top = select(WbFunnelDaily).where(
        WbFunnelDaily.date == td,
    )
    ...
    # ── Trend data (last N days aggregated by date) ───
    q_trend = select(
        WbFunnelDaily.date,
    ...
    # ── Anomalies ───
    # Compare each product today vs its average over the trend window
    q_avg = select(
        WbFunnelDaily.nm_id,
    ...
    avg_map = {r.nm_id: {"avg_sum": float(r.avg_orders_sum or 0), "avg_cnt": float(r.avg_orders_count or 0)} for r in avg_rows}
    ...
    for p in top_products:
        ...
        if avg["avg_sum"] > 0:
            ...
            if abs(pct_change) > 30:
                flags.append(f"{'📈' if pct_change > 0 else '📉'} Выручка {'+' if pct_change > 0 else ''}{round(pct_change)}% vs средней")
        if p["drr"] > 20:
