"""
Cost Excel normalizers — one function per supplier format.
Extracted from cost_parsers.py for maintainability.
"""
...
def normalize_divandek(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["area_m2"] = 0
    df["barcode"] = (
        pd.to_numeric(df.get("barcode", pd.Series(dtype="str")), errors="coerce").fillna(0).astype(int).astype(str)
    )
...
def normalize_divandek_cn(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "size" in df.columns:
        df["area_m2"] = df["size"].apply(_area_from_size_divandek)
    else:
        df["area_m2"] = 0
...
def _area_from_size_divandek(s):
    """Parse area from size string like '180*200' or '*240 + 50*7'."""
    try:
        s = str(s).strip()
        if "*" in s:
