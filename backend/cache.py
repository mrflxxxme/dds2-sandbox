...
async def invalidate_project_reports(project_id: int):
    ...
    for prefix in (
        ...
        "funnel:tariff_map",
        "funnel:avg_buyout",
        "reports:logistics_analytics_v2",
        "reports:logistics_cost_per_unit",
        "reports:logistics_forecast",
        ...
        # FF billing (тарифы ФФ, хранение по дням, сверка счетов)
        "ff_billing:tariffs",
        "ff_billing:storage",
        "ff_billing:invoices",
        ...
        # accruals2 — помесячный ФОТ с разбивкой для ОПиУ (v2: dict вместо строки,
        # старые ключи payroll:accruals дочистит TTL); scope_options — DISTINCT
        # по wb_finance_rows)
        "payroll:sheet",
        "payroll:accruals2",
