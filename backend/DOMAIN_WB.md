...
- **Проблемные новинки** (`GET /reviews/newcomers?days=30&max_rating=4.6`, `get_new_low_rated`): товары «на продаже» < `days` дней со средним рейтингом < `max_rating`. «Старт продаж» = `Nomenclature.first_sale_date`, при NULL — фолбэк на дату ПЕРВОГО отзыва (прокси). Вкладка «🆕 Проблемные новинки». Не кэшируется.
