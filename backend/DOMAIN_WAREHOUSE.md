...
- `GET /warehouse/transfers` — список перемещений; `GET /warehouse/transfers/{id}` — карточка; `POST /warehouse/transfers` — создание; `POST /warehouse/transfers/{id}/send` — отправка (DRAFT→IN_TRANSIT, создаёт забор); `POST /warehouse/transfers/{id}/complete` — завершение (IN_TRANSIT→COMPLETED, приходует на target); `DELETE /warehouse/transfers/{id}` — отмена (только из DRAFT, soft-delete).
