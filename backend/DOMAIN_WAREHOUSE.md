...
- **Правка перемещения (`update_transfer`, `PUT /transfers/{id}`, 2026-07-31): ТОЛЬКО в DRAFT.** Меняются маршрут (`from_warehouse_id`/`to_warehouse_id` — в черновике сток ещё не двигался), комментарий, брак, транспортная единица и состав. `items` — ПОЛНАЯ ЗАМЕНА (резолв по баркодам, как в `create_transfer`); не передан — состав не трогаем, передан пустым — 400 (переезд без позиций всё равно не отправить, а молча стереть состав хуже отказа). Применяются только ЯВНО переданные поля (`model_dump(exclude_unset=True)` в роутере): частичное тело не обнуляет остальное дефолтами схемы, `shipped_as_boxes` здесь обычный bool (трёхзначный — только в `TransferAssignVehicle`). Вся валидация идёт ДО первой мутации: отказ на резолве ШК не должен оставить наполовину применённую правку. В IN_TRANSIT/COMPLETED — 400: сток уже списан и снят снимком в забор, правка состава разъехалась бы с движениями. 🔴 **Гард смены маршрута:** если у переезда есть связки ФФ, чей склад после правки перестаёт быть концом маршрута — отказ с номерами заявок. `link_request` такую пару создать бы не дал, а `_collect_transfer_fact_candidates` отбирает приёмки ПО СКЛАДУ ЗАЯВКИ и джойном конец маршрута не сверяет — факт чужого склада поехал бы на новый `to_warehouse_id`.
- **Связки ФФ в карточке (`ff_links`):** `get_transfer` (и ответ `PUT`) отдаёт уже привязанные заявки ФФ обеих сторон одной выборкой по `stock_transfer_id` (`fulfillment_service.list_transfer_ff_links`), с `warehouse_id` и `side` в каждой строке. В СПИСКЕ поле не заполняется вовсе (атрибут не ставится, схема берёт дефолт `[]`). Раньше карточка ради 0-2 связок тянула ДВА полных списка заявок ФФ по обоим складам (~300 КБ); на складе «Натали» уже 432 заявки при лимите 500 — связка вот-вот перестала бы находиться. Это про корректность, а не про скорость.
...
- `GET /warehouse/transfers` — список перемещений; `GET /warehouse/transfers/{id}` — карточка (с `items` и `ff_links`); `POST /warehouse/transfers` — создание; `PUT /warehouse/transfers/{id}` — правка (**только DRAFT**, см. ниже); `POST /warehouse/transfers/{id}/send` — отправка (DRAFT→IN_TRANSIT, создаёт забор); `POST /warehouse/transfers/{id}/complete` — завершение (IN_TRANSIT→COMPLETED, приходует на target); `DELETE /warehouse/transfers/{id}` — отмена (только из DRAFT, soft-delete).
...
- `POST /warehouse/transfers/{id}/assign-vehicle` — назначить машину на переезд (в DRAFT, с логистикой: машина, водитель, перевозчик, даты, стоимость, единица груза). `POST /warehouse/transfers/assign-vehicle-bulk` — одна машина на N переездов. `POST /warehouse/transfers/{id}/unassign-vehicle` — снять машину (чистит логистику, не трогает единицу груза).
