...
class FfServiceType(str, enum.Enum):
    ...
    #: Сборка ПЕРЕЕЗДА между складами, ₽/паллета ИЛИ ₽/короб (unit).
    #: Отдельная ставка, а не переиспользование PALLETIZING/BOX_PROCESSING:
    #: переезд для ФФ — другая работа (собрать и отдать машине без маркетплейсной
    #: подготовки), и склад берёт за неё свою цену (канон юзера 01.08.2026).
    #: Общая для обеих сторон маршрута: тариф лежит на складе, а переезд его
    #: склада-ИСТОЧНИКА и есть тот, кто эту работу выполняет.
    TRANSFER_ASSEMBLY = "TRANSFER_ASSEMBLY"
