...
## Ownership
Lead-agent + supply-chain команда. Все изменения в `services/supply_chain/*`, `routers/supply_chain.py`, `models/supply_chain.py`, `models/cost.py` (vehicle extension) идут через DOMAIN_SUPPLY_CHAIN.md.
...
## Dependencies
- `DOMAIN_COST` — CostOrder / CostOrderItem (расширены для машин)
- `DOMAIN_WAREHOUSE` — InboundReceipt auto-create при DELIVERED, warehouse_stock_engine._next_number
- `DOMAIN_REPORTS` — invalidate_project_reports при пересчёте
