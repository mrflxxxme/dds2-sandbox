...
class MigfullPackingLine(BaseModel):
    ...
    ``qty`` — отправляемое количество: может быть МЕНЬШЕ строки приёмки
    (частичная отправка), больше — 400; 0 — строка не едет (опись пропустит).
    """

    barcode: str = Field(min_length=1, max_length=100)
    qty: int  # 0..кол-во строки приёмки (валидация в сервисе → 400, не 422)
