...
# У кампаний с ЕДИНОЙ ставкой CPM пофразовая ставка недоступна: одна ставка правит всеми
# фразами, WB отвечает 200, но override по фразе игнорирует. МИНУС-ФРАЗЫ при этом работают
# (исключать/возвращать кластеры WB принимает) — гейтим только ставки.
UNIFIED_CLUSTER_LOCK = (
    "У кампании единая ставка CPM — одна ставка правит всеми кластерами, менять ставку "
    "по отдельной фразе WB не даёт. Переключите кампанию на ручные ставки (по зонам) "
...
async def toggle_cluster_minus(
    ...
    # bid_mode здесь НЕ проверяем: минус-фразы WB принимает и у единой ставки CPM
    nm_ids = [int(n) for n in (camp.nm_ids or [])]
