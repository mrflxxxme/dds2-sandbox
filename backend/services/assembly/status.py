...
# удалять нельзя (нужен ручной откат стока через отмену). PRE_DISTRIBUTED тоже сюда:
# это резерв под машину в пути, им управляют на экране «Распределить машину» (не на
# общем списке), а в списке такие строки теперь выбираемы ради «Объединить» — без
# этого гарда клик по «Удалить выбранные» тихо снёс бы предраспределение.
