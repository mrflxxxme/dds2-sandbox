...
def greedy_allocate_to_target(
    ...
    for wh in order:
        ...
        take = int(min(
            ...
            # ceil, не floor: спрос ФО дробный (avg_daily × горизонт), а грузим
            # целыми штуками. floor(0.9)=0 давал НОЛЬ long-tail SKU во всех
            # клетках при can_send>0 и терял до 1 шт на каждый ФО у всех SKU.
            # Перелив ≤ дробной части (<1 шт/ФО); после налива headroom≤0 —
            # следующие склады того же ФО пропускаются (не перетаривать).
            math.ceil(headroom),    # не перетарить округ сверх спроса (целыми шт)
