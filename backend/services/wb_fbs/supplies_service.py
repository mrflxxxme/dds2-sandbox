...
def _order_ids_candidates() -> Any:
    ...
    • **активные** (`done = false`) — каждый прогон, СКОЛЬКО БЫ заданий у нас
      уже ни было: в такую поставку докладывают прямо в кабинете, состав живой,
      и только свежая метка даёт право предложить её к доклажу
      (`_load_active_supply_fits`);
    ...
    Условие `orders_count == 0` у активной ветки было ВТОРОЙ половиной той же
    ошибки, зеркальной первой: привязав ПЕРВОЕ задание, поставка выпадала из
    досинка навсегда, и всё доложенное после оставалось без `supply_id`. Прод
    02.08.2026: `WB-GI-260717413` показывала 5 заданий против 203 в кабинете.
    Стоимость снятия — один запрос на активную поставку за прогон (их единицы,
    и общий потолок `_ORDER_IDS_FETCH_CAP` их всё равно ограничивает).
    """
    return and_(
