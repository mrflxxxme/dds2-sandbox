...
Инварианты, которые легко потерять:
  • Обе стороны источника — то, что РЕАЛЬНО МОЖНО ОТГРУЗИТЬ. Наш ledger таким
    ...
    `fulfillment_service.ff_unavailable_by_nomenclature`): собранное под чужую
    отгрузку и идущее в приёмке — то, что провайдер сам не считает свободным.
    Собранное под НАШИ активные заявки туда не входит — его убирает `reserved`,
    иначе вычли бы дважды.
    Недоступное ≠ брак, и брак из зеркала НЕ вычитается: у skladbot `qty_good`
    (`amount`) уже без брака (`repair_amount` — отдельная корзина), вычет был
    двойным и резал живой остаток (прод-баг 28.07.2026, склад Газпром: 491 штука
    срезана, 25 позиций схлопнуты в ноль; 160х230_вишня — годного 8, брака 14 →
    зеркало 0). У migfull поля брака в API нет вовсе, и в заблокированном лежит
    в том числе товар на сборке. Поэтому колонка «Брак» показывает только
...
def _net_mirror(
    ...
    покупателю нельзя — собранное под чужую отгрузку, идущее в приёмке — таким
    обещанием быть не может: заказ придёт, а собрать будет нечем. Брака здесь
    нет: у skladbot он лежит отдельной корзиной, `qty_good` его не содержит.
...
async def stock_matrix(db: AsyncSession, project_id: int, trend_days: int = 14) -> dict[str, Any]:
    ...
    ЕДИНСТВЕННОЕ расхождение с «Остатками» — намеренное: FBO-гейт склада
    («отдаём только то, чего нет на FBO») в матрице НЕ применяется. Гейт —
    правило трансляции, а матрица — планирование физических поставок; с гейтом
    включение настройки на складе срезало «Можем» в ноль по всем позициям с
    живым FBO (жалоба 30.07). Для придержанных строк берётся `qty_computed`.
    ...
    for wh in warehouses:
        ...
        for r in preview.get("rows", []):
            ...
            # Гейт «только то, чего нет на FBO» — правило ТРАНСЛЯЦИИ, не физики:
            # он придерживает передачу в WB, но товар на складе есть. Матрица
            # отвечает «что могли бы поставить», и включение гейта на складе
            # срезало ей «Можем» в ноль по всем позициям с живым FBO. Для
            # придержанных гейтом строк берём qty_computed — тот же расчёт со
            # всеми вычетами (проданное FBS, буфер, потолки), но до гейта и
            # ручного количества.
            if r.get("blocked_reason") == BLOCKED_FBO_IN_STOCK:
                can_qty = int(r.get("qty_computed") or 0)
