...
### Воронка продаж (`funnel.py`)
- `POST /api/v1/funnel/sync` — синхронизация данных из WB API (аналитика + реклама)
- `GET /api/v1/funnel/data` — данные воронки (агрегация по дням / детализация по артикулам)
- `GET /api/v1/funnel/filters` — фильтры (бренды, предметы)
- `GET /api/v1/funnel/costs` — себестоимость товаров
- `POST /api/v1/funnel/cost` — обновить себестоимость
...
| `wb_cost_override` | Переопределения себестоимости товаров |
