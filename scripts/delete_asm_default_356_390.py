...
"""One-off: cancel + soft-delete ASM-356..390 в проекте «Default» (id=4).

Для каждой заявки: если не CANCELLED — перевод в CANCELLED через cancel_request
(запись в status_history; остатки списываются только при SHIP, поэтому для
IN_PROGRESS/PENDING движения остатков нет, для SHIPPED — откат как в обычной
отмене) → soft-delete через delete_request. Идемпотентно: уже удалённые/
отсутствующие пропускаются.
...
PID = 4
NUMBERS = [f"ASM-{n}" for n in range(356, 391)]  # 356..390 включительно (35 шт)
