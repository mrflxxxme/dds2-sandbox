...
export function Integrations() {
    ...
    const [service, setService] = useState<'wb' | 'wb_advert' | 'wb_content' | 'wb_feedbacks' | 'wb_marketplace' | 'wb_marketplace_sandbox'>('wb');
                ...
                {showAdd && (
                    <div style={{ background: 'var(--color-bg-input)', border: '1px solid var(--color-border)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
                        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                            <div className="form-group" style={{ width: 200 }}><label className="form-label">Тип ключа</label><select className="form-input" value={service} onChange={e => setService(e.target.value as 'wb' | 'wb_advert' | 'wb_content' | 'wb_feedbacks' | 'wb_marketplace' | 'wb_marketplace_sandbox')}><option value="wb">Основной (статистика, финансы)</option><option value="wb_advert">Реклама (Продвижение)</option><option value="wb_content">Контент (карточки, АБ-тесты фото)</option><option value="wb_feedbacks">Отзывы (Вопросы и отзывы)</option><option value="wb_marketplace">Маркетплейс (FBS: заказы, остатки)</option><option value="wb_marketplace_sandbox">Маркетплейс — песочница (scope Test)</option></select></div>
                        ...
                        {service === 'wb_marketplace' && (<div style={{ fontSize: 12, color: 'var(--color-text-dim)', marginTop: 8 }}>Токен с категорией «Маркетплейс». Используется разделом «FBS Wildberries»: склады продавца, сборочные задания, трансляция остатков, поставки. Токен только на чтение подойдёт для загрузки заказов и превью остатков — запись всё равно закрыта режимом контура.</div>)}
                        {service === 'wb_marketplace_sandbox' && (<div style={{ fontSize: 12, color: 'var(--color-text-dim)', marginTop: 8 }}>Токен песочницы WB (scope Test) — работает только на marketplace-api-sandbox. Нужен, когда режим контура FBS переключён в «sandbox» для репетиции поставок и стикеров на тестовых данных.</div>)}
                    ...
                    keys.map(k => (
                        <div key={k.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--color-border)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                ...
                                <div><div style={{ fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>{k.label || 'Wildberries API'}<span className={`badge ${k.service === 'wb_advert' ? 'badge-warning' : k.service === 'wb_content' ? 'badge-success' : k.service === 'wb_marketplace' ? 'badge-success' : k.service === 'wb_marketplace_sandbox' ? 'badge-warning' : k.service === 'wb_feedbacks' ? 'badge-info' : 'badge-info'}`}>{k.service === 'wb_advert' ? 'Реклама' : k.service === 'wb_content' ? 'Контент' : k.service === 'wb_marketplace' ? 'Маркетплейс' : k.service === 'wb_marketplace_sandbox' ? 'Маркетплейс (песочница)' : k.service === 'wb_feedbacks' ? 'Отзывы' : k.service === 'wb_analytics' ? 'Аналитика' : 'Основной'}</span></div><div style={{ fontSize: 12, color: 'var(--color-text-dim)', fontFamily: 'monospace' }}>{k.key_preview || '••••••••'}</div></div>
                            </div>
