...
/** Прежняя воронка — оставлена как запасной вид на время привыкания к новой.
 *  Код снят с коммита b958e56f (до переделки 31.07.2026) и живёт своим маршрутом
 *  `/funnel/legacy`; новая воронка ссылается сюда кнопкой «Старый вариант». */
export default function LegacyFunnelPage() {
    const routeParams = useParams();
    const projectSlug = typeof routeParams?.slug === 'string' ? routeParams.slug : Array.isArray(routeParams?.slug) ? routeParams.slug[0] : '';
    ...
    return (
        <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <h1 style={{ fontSize: 22, fontWeight: 600, margin: 0 }}>📊 Воронка продаж</h1>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#92400e', background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 20, padding: '3px 10px' }}>старый вид</span>
                <Link href={`/p/${projectSlug}/funnel`} className="btn btn-secondary btn-sm" style={{ marginLeft: 'auto', fontSize: 12, textDecoration: 'none' }}>Новый вариант →</Link>
            </div>
                    ...
                    {groupBy === 'day' && <div className="glass-card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ overflow: 'auto', maxHeight: 'calc(100vh - 280px)' }}>
                            {loading ? <div style={{ padding: 40, textAlign: 'center' }}>Загрузка...</div> : (
                                ...
                                <thead>
                                    <tr ref={headerRow1Ref}>
                                        {/* backdropFilter гасит blur из `.data-table thead th`; WebkitBackdropFilter
                                            обязателен парой — Safari < 18 знает только префиксную форму, и без неё
                                            размытие на этих колонках не снимается (React префикс сам не дописывает). */}
                                        <th rowSpan={2} style={{ position: 'sticky', left: 0, top: 0, background: '#ffffff', color: '#374151', backdropFilter: 'none', WebkitBackdropFilter: 'none', zIndex: 22, verticalAlign: 'bottom', borderBottom: '2px solid #e5e7eb', minWidth: 100, borderRight: '1px solid #e5e7eb', padding: '8px 12px', boxShadow: !detailed ? 'inset -6px 0 6px -6px rgba(0,0,0,0.08)' : 'none' }}>ДАТА</th>
                                        {detailed && <th rowSpan={2} style={{ position: 'sticky', left: 100, top: 0, background: '#ffffff', color: '#374151', backdropFilter: 'none', WebkitBackdropFilter: 'none', zIndex: 22, verticalAlign: 'bottom', minWidth: 130, borderRight: '1px solid #e5e7eb', padding: '8px 12px', boxShadow: 'inset -6px 0 6px -6px rgba(0,0,0,0.08)' }}>Артикул</th>}
