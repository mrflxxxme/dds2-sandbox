...
export default function EventsCard({ task }: { task: DesignTaskDetail }) {
    ...
    return (
        <div className="glass-card">
            ...
            {events.length === 0 && (
                <div style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>Переходов ещё не было.</div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {events.map((e) => (
                    <div key={e.id} style={{ fontSize: 13, borderBottom: '1px solid var(--color-border)', paddingBottom: 6 }}>
                        <span style={{ color: 'var(--color-text-muted)' }}>{formatDateTime(e.changed_at)}</span>
                        {' · '}
                        <span>
                            {e.old_status ? `${DESIGN_STATUS_LABEL[e.old_status]} → ` : ''}
                            <b>{DESIGN_STATUS_LABEL[e.new_status]}</b>
                        </span>
                        {e.changed_by && <span style={{ color: 'var(--color-text-muted)' }}> · {e.changed_by}</span>}
                        {e.comment && (
                            <div style={{ color: 'var(--color-text-muted)', whiteSpace: 'pre-wrap', marginTop: 2 }}>{e.comment}</div>
                        )}
                    </div>
                ))}
            </div>
