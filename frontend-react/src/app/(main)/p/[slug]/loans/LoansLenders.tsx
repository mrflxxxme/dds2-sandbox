...
export default function LoansLenders({ nonce, onChanged }: { nonce: number; onChanged: () => void }) {
    ...
    const load = useCallback(async () => {
        ...
        setError('');
        try {
            const [byLender, accessList] = await Promise.all([api.loansByLender(), api.listLenderAccess()]);
            setData(byLender);
    ...
    const columns = [
        { key: 'name', label: 'Заёмщик', render: (v: string) => <span style={{ fontWeight: 600 }}>{v}</span> },
        {
        ...
        { key: 'accrued_interest', label: 'Начислено %', align: 'right' as const, getValue: (r: LoanLenderRollup) => Number(r.accrued_interest), render: (_v: unknown, r: LoanLenderRollup) => `${money(r.accrued_interest)} ₽` },
        ...
        <div>
            {data && (
                <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                    <div className="glass-card" style={{ padding: '12px 18px', flex: '1 1 200px' }}>
                        <div style={{ fontSize: 12, color: 'var(--color-text-dim)' }}>Всего в займах</div>
                        <div style={{ fontSize: 22, fontWeight: 700 }}>{money(data.total_outstanding)} ₽</div>
                    </div>
                    <div className="glass-card" style={{ padding: '12px 18px', flex: '1 1 200px' }}>
                        <div style={{ fontSize: 12, color: 'var(--color-text-dim)' }}>Начислено процентов (период)</div>
                        <div style={{ fontSize: 22, fontWeight: 700 }}>{money(data.total_accrued)} ₽</div>
                    </div>
                    <div className="glass-card" style={{ padding: '12px 18px', flex: '1 1 200px' }}>
                        <div style={{ fontSize: 12, color: 'var(--color-text-dim)' }}>Заёмщиков</div>
                        <div style={{ fontSize: 22, fontWeight: 700 }}>{data.items.length}</div>
                    </div>
                </div>
            )}
            ...
            {loading ? (
                ...
                <div className="glass-card" style={{ padding: 48, textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 600 }}>Заёмщиков пока нет</div>
                </div>
            ) : (
                <TanStackDataTable columns={columns} data={data?.items ?? []} exportName="lenders" enableSorting enablePagination pageSize={50} />
            )}
