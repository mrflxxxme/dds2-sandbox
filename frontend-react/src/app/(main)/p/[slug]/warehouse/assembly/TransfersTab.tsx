...
/**
 * Переезд ушёл в архив: убран РУКАМИ, либо завершён, либо везёт брак.
 *
 * Три источника, и первый главный: `archived` — это решение человека («В архив»
 * на карточке), и оно перебивает всё остальное. Живой по статусу переезд,
 * заведённый по ошибке, обязан уходить с глаз, а два вычисляемых признака его
 * бы не поймали.
 *
 * Брак и завершённые статусы — вычисляемые признаки (требование юзера
 * 01.08.2026): они ездят отдельным потоком и в рабочем списке только мешают.
 * Переезд при этом НЕ прячется совсем — вид «Архив»/«Все» его показывает, а
 * счётчик в шапке считает всё.
 */
function isTransferArchived(t: StockTransfer): boolean {
    return t.archived === true
        || TRANSFER_ARCHIVE_STATUSES.has(t.status)
        || t.is_defect === true;
}
