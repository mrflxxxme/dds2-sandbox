        ...
        <div className="animate-in">
            ...
            <div className="page-header">
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {/* Локальный архив — «убрать с глаз», а не статус. Доступен
                        В ЛЮБОЙ стадии: в архив просят и живой переезд (завели по
                        ошибке, дубль), и давно принятый. Сток и деньги остаются
                        на месте, поэтому кнопка безопасна везде. */}
                    {editor && (
                        <button
                            className="btn btn-secondary"
                            onClick={() => runAction(
                                () => (transfer.archived
                                    ? api.unarchiveTransfer(transfer.id)
                                    : api.archiveTransfer(transfer.id)),
                                transfer.archived ? 'Переезд возвращён из архива' : 'Переезд убран в архив',
                            )}
                            disabled={actionLoading}
                            title={transfer.archived
                                ? 'Вернуть в рабочие списки'
                                : 'Убрать из рабочих списков. Статус и сток не изменятся, в отчётах и «Оплатах» переезд останется'}
                        >
                            {transfer.archived ? 'Вернуть из архива' : 'В архив'}
                        </button>
                    )}
            ...
            <div className="glass-card" style={{ padding: 20, marginBottom: 16 }}>
                ...
                <FfSideSection
                    ...
                    emptyText={transfer.converted_from_assembly_id != null
                        ? 'Заявок ФФ на отгрузку не связано. Незавершённые заявки провайдера переезжают сюда сами, когда переделка отменяет заявку на сборку; завершённые остаются её историей — их переносит галка «перенести связку ФФ». Нажмите «Связать» — покажем свободные сборки этого склада.'
